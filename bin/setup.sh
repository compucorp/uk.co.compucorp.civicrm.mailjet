...
if [ ! -e "$CIVIROOT/xml/schema/Mailjet" ] ; then
  ln -s $EXTROOT/xml/schema/Mailjet $CIVIROOT/xml/schema/Mailjet
fi
cd $CIVIROOT/xml
php GenCode.php
# (There may be extra arguments to pass into GenCode.php; not sure)

cp -f $CIVIROOT/CRM/Mailjet/DAO/* $EXTROOT/CRM/Mailjet/DAO/
mv $CIVIROOT/xml/schema/Schema.xml.backup $CIVIROOT/xml/schema/Schema.xml

unlink $CIVIROOT/xml/schema/Mailjet
rm -rf $CIVIROOT/CRM/Mailjet
